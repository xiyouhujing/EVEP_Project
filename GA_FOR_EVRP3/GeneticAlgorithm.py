...
class GeneticAlgorithm:
    ...
    def __init__(self, dis_dep_cus, dis_sta_cus, dis_cus_cus, min_dis_depot, min_dis_station, demand, time_win, tolerate_time_win, mans, rows, times):
        ...
        self.min_dis_depot = min_dis_depot  # customer到哪个depot最近
    ...
    def run(self):
        ...
        for i in range(0, self.rows):
            line = random.sample(range(1, self.mans+1), self.mans)
            lines[i] = line

            # 计算每个线路的适应度
            # print "计算每个线路的适应度 i = %d" % (i)
            fit[i] = calFitness.fitness(self.dis_cus_cus, self.dis_sta_cus, self.min_dis_depot, self.min_dis_station, self.time_win, self.tolerate_time_win, self.demand, lines[i], False)
        ...
        while t < self.times:
            ...
            # 对最优解使用爬山算法促使其自我进化
            clMountain.clMountain(self.dis_cus_cus, self.dis_sta_cus, self.min_dis_depot, self.min_dis_station, self.time_win, self.tolerate_time_win, self.demand, newLines[0])
            ...
            while nl < self.rows:
                ...
                if random.random() < self.JCL and nl + 1 < self.rows:
                    ...
                    nextFit[nl] = calFitness.fitness(self.dis_cus_cus, self.dis_sta_cus, self.min_dis_depot, self.min_dis_station, self.time_win, self.tolerate_time_win, self.demand, fline, False)
                    nextFit[nl + 1] = calFitness.fitness(self.dis_cus_cus, self.dis_sta_cus, self.min_dis_depot, self.min_dis_station, self.time_win, self.tolerate_time_win, self.demand, nline, False)
                    ...
                    nextFit[nl] = calFitness.fitness(self.dis_cus_cus, self.dis_sta_cus, self.min_dis_depot, self.min_dis_station, self.time_win, self.tolerate_time_win, self.demand, line, False)
                    nl += 1
        ...
        print "迭代完成"
        # 输出结果:
        calFitness.fitness(self.dis_cus_cus, self.dis_sta_cus, self.min_dis_depot, self.min_dis_station, self.time_win, self.tolerate_time_win, self.demand, lines[ml], True)
