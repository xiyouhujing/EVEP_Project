...
def fitness(dep_num, dis_cus_cus, dis_sta_cus, dis_dep_cus, min_dis_sta, time_win, tolerate_time_win, demand, line, isShow):
    ...
    carDis = dis_dep_cus[fore-1]  # 当前车辆行驶的总距离
    ...
    newDis = dis_dep_cus[fore-1]
    totalDis = 0
    Cost = 0

    route.append(dep_num)
    r = 1       # 表示当前需要车辆数
    ...
    while j < len(line):
        ...
        if newDis > dianliang/haodian and newTon <= zaizhong and arriverTime <= tolerate_time_win[line[j]-1]:
            ...
            newDis = dis_sta2cus  # 车辆从充电站到line[j]的行驶距
            arriverTime = carTime + (dis_cus2sta + dis_sta2cus) / sudu + chongdian_time  # 车辆从当前点到充电站,再从充电站到line[j]的到达时间

            if (newDis - dis_cus_cus[fore-1][line[j]-1] + dis_cus2sta > dianliang/haodian and newDis - dis_cus_cus[fore-1][line[j]-1] + dis_dep_cus[fore-1] < dianliang/haodian) or arriverTime > tolerate_time_win[line[j]-1]:
                # 不去充电站，返回depot
                totalDis += carDis + dis_dep_cus[fore - 1]
                Cost += cf_cost
                route.append(dep_num)
                route.append([cf_cost + (carDis + dis_dep_cus[fore - 1]) * c2 + c1, cf_cost,
                              (carDis + dis_dep_cus[fore - 1]) * c2])
                routes.append(route)

                r += 1  # 换一辆车配送line[j]
                fore = line[j]  # 当前配送客户点为line[j]
                carTon = demand[fore - 1]
                carDis = dis_dep_cus[fore - 1]
                ...
                carTon = newTon
                cf_cost += a * max(time_win[line[j] - 1][0] - arriverTime, 0) + b * max(
                    arriverTime - time_win[line[j] - 1][1], 0)
                fore = line[j]
                route.append(sta_node + 1 + 44)

            # 如果充电后到达line[j]的时间超过了其接受时间窗，从当前点返回最近的depot
            # if arriverTime > tolerate_time_win[line[j]-1]:
            #     totalDis += carDis + dis_dep_cus[dep_num][fore-1]
            #     Cost += cf_cost
            #     route.append(depot_l+1+mans)
            #     route.append(0)
            #     route.append([cf_cost+(carDis + min_dis_depot[fore - 1][1])*c2+c1, cf_cost, (carDis + dis_dep_cus[depot_s][fore-1])*c2])
            #     routes.append(route)
            #
            #     r += 1  # 换一辆车配送line[j]
            #     fore = line[j]  # 当前配送客户点为line[j]
        ...
        # 如果到达j点后的载重超过最大载重，并且去depot的距离没有超过depot，换下一辆车
        elif newTon > zaizhong or arriverTime > tolerate_time_win[line[j]-1]:
            totalDis += carDis + dis_dep_cus[fore-1]
            Cost += cf_cost
            route.append(dep_num)
            route.append([cf_cost+(carDis + dis_dep_cus[fore-1])*c2+c1, cf_cost, (carDis + dis_dep_cus[fore-1])*c2])
            routes.append(route)
