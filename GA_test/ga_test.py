...
class GeneticAlgorithm():
    ...
    def __init__(self, dist, dema, e_time, l_time, ee_time, ll_time, rows, times, depots, mans, cars, tons, distance, server, PW):
        self.dist = dist
        self.dema = dema
        self.et = et
        self.lt = lt
    ...
    def run(self):
        ...
        #顾客点包括depots和mans
        customers = self.depots + self.mans

        #路线数组
        lines = [[0 for i in range(0, customers)] for i in range(self.rows)]
        ...
        j = 0
        depots_m = [i for i in range(1, self.depots+1)]
        for i in range(0, self.rows):
            j = 0
            while j < customers:
                num = int(random.uniform(0, customers))+1
            ...
            fit[i] = self.calFitness(lines[i], customers, False)
    ...
    def calFitness(self, line, customers, isShow,):
        cd_dis = self.dist[:customers+1, :self.depots+1]
        cf_dis = self.dist[:customers+1, customers:]
        cc_dis = self.dist[:customers+1, :customers+1]
        ...
        r = 0       #表示当前需要车辆数
        # l = 0
        # fore = int(random.uniform(0, self.depots))    #表示正在运送的客户编号
        M = 0       #表示当前的路径规划所需要的总车辆和总共拥有的车辆之间的差，如果大于0，表示是一个失败的规划，乘以一个很大的惩罚因子用来降低适应度

        #遍历每个客户点
        i = 0
        first_c = line[i]
        min_dc = min(cd_dis[first_c-1])
        fore = cd_dis[first_c-1].index(min_dc)+1
        while i < customers:
            #行驶的距离
            newDis = carDis + cc_dis[fore-1][line[i]-1]

            #当前车辆的载重
            newTon = carTon + self.dema[line[i]-1]

            #如果已经超过最大载重，换下一辆车
            if newTon > self.tons:
                #寻找离当前客户最近的depot点返回
                min_cd_dis = min(cd_dis[fore-1])
                min_cd_index = cd_dis.index(min_cd_dis)
                totalDis += carDis + cd_dis[fore-1][min_cd_index]  #后面加这个d[fore][0]表示需要从当前客户处返程的距离
                r += 1
                next_c = line[i]
                fore = cd_dis[next_c-1].index(min(cd_dis[next_c-1]))+1
                i -= 1  #表示当前这个点的配送还没有完成
                carTon = 0
                carDis = 0
            if newDis > self.distance:
                min_cf_dis = min(cf_dis[fore-1])
                min_cf_index = cf_dis[fore-1].index(min_cf_dis)
                totalDis += carDis + cf_dis[fore-1][min_cf_index]
                fore = min_cf_index+1
                i -= 1
                carDis = 0
        ...
        #加上最后一辆车的距离和返程的距离
        totalDis += carDis + self.dist[fore][0]
        ...
        result = 1 / (self.c1*(r+1) + totalDis*self.c2)
...
ga = GeneticAlgorithm(dist=d, dema=q, e_time=et, l_time=lt, ee_time=eet, ll_time=llt, rows=5, times=5, depots=3, mans=34, cars=999, tons=100, distance=150, server=10, PW=10000)
